...
class ECFDSpatial(BaseDecodeHead):
    def __init__(self,
        ...
        super(ECFDSpatial, self).__init__(**kwargs)
        ...
        self.RCE = ReinforceContextEmbedding(
            ...
            in_chans=[self.channels, embed_dim, embed_dim, embed_dim], is_fuse=True, align_corners=self.align_corners)
    ...
    def forward(self, inputs):
        ...
        deform_inputs = self.deform_inputs(inputs)
        
        for i in range(self.L):
            c = self.blocks[i](c, deform_inputs, space_shape)
...
class ReinforceContextEmbedding(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=[256, 512, 1024, 2048], patch_ratio=2,
        ...
        if self.is_fuse:
            ...
            self.fuse_layer = nn.Conv2d(in_chans[0] * 3, in_chans[0], kernel_size=1)
        else:
...
class CrossLayerFusionBlock(nn.Module):
    ...
    # space_shape = [inputs[1].shape[2:], inputs[2].shape[2:], inputs[3].shape[2:]]
    def forward(self, F_sp, deform_input, space_shape):
        F_sp = self.GeneralFusion(deform_input[0], F_sp, deform_input[1], deform_input[2], space_shape)
        return F_sp
...
class GeneralFusionModule(nn.Module):
    ...
    def forward(self, reference_points, feat, spatial_shapes, level_start_index, space_shape):
        # print('reference_points: {}'.format(reference_points))
        # print('feat.shape: {}'.format(feat.shape))
        # print('spatial_shapes: {}'.format(spatial_shapes))
        # print('level_start_index: {}'.format(level_start_index))
        # print('space_shape: {}'.format(space_shape))
        # exit()
